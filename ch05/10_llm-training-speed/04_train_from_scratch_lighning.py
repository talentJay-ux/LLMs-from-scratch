...
def evaluate_model(model, train_loader, val_loader, device, eval_iter, tokenizer, start_context):
    model.eval()
    with torch.no_grad():
    ...
    for epoch in range(num_epochs):
        ...
        for inp_batch, tgt_batch in train_loader:
            ...
            if global_step % eval_freq == 0:
                ...
                train_loss, val_loss = evaluate_model(model, train_loader_fixed, val_loader_fixed, device, eval_iter, tokenizer, start_context)
...
class LitGPT(L.LightningModule):
    ...
    def forward(self, x):  # for generation
        return self.model(x)

    def training_step(self, batch, batch_idx):
        inp, tgt = batch  
        logits = self.model(inp)
        loss = nn.functional.cross_entropy(logits.flatten(0, 1), tgt.flatten())
        tokens = inp.numel()
        self.log("train_loss", loss, prog_bar=False, on_step=True, on_epoch=True)
        self.log("tokens_step", float(tokens), prog_bar=False, on_step=True, logger=True)
        return loss
    ...
    def configure_optimizers(self):
        opt = torch.optim.AdamW(self.parameters(), lr=self.lr, weight_decay=self.wd, fused=True)
        return opt
...
def main(gpt_config, settings):
    ...
    lit = LitGPT(base_model=gpt_model, lr=settings["learning_rate"], weight_decay=settings["weight_decay"])
    ...
    train_losses = [float(trainer.callback_metrics["train_loss_epoch"])] if "train_loss_epoch" in trainer.callback_metrics else []
    val_losses   = [float(trainer.callback_metrics["val_loss"])] if "val_loss" in trainer.callback_metrics else []

    return train_losses, val_losses, gpt_model
...
if __name__ == "__main__":
    ...
    train_losses, val_losses, model = main(GPT_CONFIG_124M, OTHER_SETTINGS)
